...
func (_q *WorkspaceQuery) loadChannels(ctx context.Context, query *ChannelQuery, nodes []*Workspace, init func(*Workspace), assign func(*Workspace, *Channel)) error {
	...
	for _, n := range neighbors {
		fk := n.workspace_channels
		if fk == nil {
			return fmt.Errorf(`foreign-key "workspace_channels" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "workspace_channels" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
