...
func (_q *MessageQuery) QueryThread() *MessageQuery {
...
func (_q *MessageQuery) WithThread(opts ...func(*MessageQuery)) *MessageQuery {
	query := (&MessageClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *MessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Message, error) {
	...
	if query := _q.withThread; query != nil {
		if err := _q.loadThread(ctx, query, nodes, nil,
			func(n *Message, e *Message) { n.Edges.Thread = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *MessageQuery) loadThread(ctx context.Context, query *MessageQuery, nodes []*Message, init func(*Message), assign func(*Message, *Message)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Message)
	for i := range nodes {
		if nodes[i].ThreadID == nil {
			continue
		}
		fk := *nodes[i].ThreadID
		if _, ok := nodeids[fk]; !ok {
...
func (_q *MessageQuery) loadReplies(ctx context.Context, query *MessageQuery, nodes []*Message, init func(*Message), assign func(*Message, *Message)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(message.FieldThreadID)
	}
	...
	for _, n := range neighbors {
		fk := n.ThreadID
		if fk == nil {
			return fmt.Errorf(`foreign-key "thread_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (_q *MessageQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withThread != nil {
			_spec.Node.AddColumnOnce(message.FieldThreadID)
		}
	}
