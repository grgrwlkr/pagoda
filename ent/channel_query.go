...
func (_q *ChannelQuery) QueryCreator() *UserQuery {
...
func (_q *ChannelQuery) WithCreator(opts ...func(*UserQuery)) *ChannelQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ChannelQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Channel, error) {
	...
	if query := _q.withCreator; query != nil {
		if err := _q.loadCreator(ctx, query, nodes, nil,
			func(n *Channel, e *User) { n.Edges.Creator = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *ChannelQuery) loadCreator(ctx context.Context, query *UserQuery, nodes []*Channel, init func(*Channel), assign func(*Channel, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Channel)
	for i := range nodes {
...
func (_q *ChannelQuery) loadMembers(ctx context.Context, query *ChannelMemberQuery, nodes []*Channel, init func(*Channel), assign func(*Channel, *ChannelMember)) error {
	...
	for _, n := range neighbors {
		fk := n.channel_members
		if fk == nil {
			return fmt.Errorf(`foreign-key "channel_members" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "channel_members" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *ChannelQuery) loadMessages(ctx context.Context, query *MessageQuery, nodes []*Channel, init func(*Channel), assign func(*Channel, *Message)) error {
	...
	for _, n := range neighbors {
		fk := n.channel_messages
		if fk == nil {
			return fmt.Errorf(`foreign-key "channel_messages" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "channel_messages" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *ChannelQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withCreator != nil {
			_spec.Node.AddColumnOnce(channel.FieldCreatedBy)
		}
	}
