...
type AttachmentQuery struct {
	...
	withDmContent *DirectMessageContentQuery
...
// QueryDmContent chains the current query on the "dm_content" edge.
func (_q *AttachmentQuery) QueryDmContent() *DirectMessageContentQuery {
...
func (_q *AttachmentQuery) Clone() *AttachmentQuery {
	...
	return &AttachmentQuery{
		...
		withDmContent: _q.withDmContent.Clone(),
...
// WithDmContent tells the query-builder to eager-load the nodes that are connected to
// the "dm_content" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *AttachmentQuery) WithDmContent(opts ...func(*DirectMessageContentQuery)) *AttachmentQuery {
	query := (&DirectMessageContentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withDmContent = query
	return _q
}
...
func (_q *AttachmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Attachment, error) {
	...
	if query := _q.withDmContent; query != nil {
		if err := _q.loadDmContent(ctx, query, nodes, nil,
			func(n *Attachment, e *DirectMessageContent) { n.Edges.DmContent = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *AttachmentQuery) loadDmContent(ctx context.Context, query *DirectMessageContentQuery, nodes []*Attachment, init func(*Attachment), assign func(*Attachment, *DirectMessageContent)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Attachment)
	for i := range nodes {
		if nodes[i].DmContentID == nil {
			continue
		}
		fk := *nodes[i].DmContentID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	query.Where(directmessagecontent.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "dm_content_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *AttachmentQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withDmContent != nil {
			_spec.Node.AddColumnOnce(attachment.FieldDmContentID)
		}
