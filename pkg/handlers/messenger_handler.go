...
func (h *Messenger) Routes(g *echo.Group) {
	...
	g.GET("/message/:id/thread/panel", h.MessageThreadPanel).Name = routenames.MessengerMessageThreadPanel
...
func (h *Messenger) ChannelView(ctx echo.Context) error {
	...
	// Исключаем ответы в тредах (thread_id IS NULL) - они должны отображаться только в панели треда
	...
	messages, err := h.orm.Message.
		...
		Where(message.ThreadIDIsNil()).          // Фильтр: исключаем ответы в тредах (только основные сообщения)
...
func (h *Messenger) ChannelMessages(ctx echo.Context) error {
	...
	// Get total count (excluding thread replies)
	total, err := h.orm.Message.
		Query().
		Where(message.ChannelIDEQ(id)).
		Where(message.ThreadIDIsNil()). // Exclude thread replies
	...
	// Get messages with pagination (excluding thread replies)
	messages, err := h.orm.Message.
		Query().
		Where(message.ChannelIDEQ(id)).
		Where(message.ThreadIDIsNil()). // Exclude thread replies - they should only appear in thread panel
...
// MessageThreadPanel renders the thread panel in the right sidebar.
// Shows the parent message and all replies in a panel that opens on the right side.
// Parameters:
//   - ctx: Echo context with message ID in URL params
//
// Returns:
//   - HTML fragment with thread panel or error
func (h *Messenger) MessageThreadPanel(ctx echo.Context) error {
	logger := log.Ctx(ctx)
	...
	logger.Info("Opening thread panel", "message_id", id)

	user := ctx.Get(context.AuthenticatedUserKey).(*ent.User)
	logger.Info("User opening thread panel", "user_id", user.ID, "user_name", user.Name)
	...
	if err != nil {
		if ent.IsNotFound(err) {
			logger.Warn("Thread panel failed: message not found", "message_id", id)
			return echo.NewHTTPError(http.StatusNotFound, "message not found")
		}
	...
	if !isMember {
		logger.Warn("Thread panel failed: user not a channel member", "message_id", id, "channel_id", parentMsg.ChannelID, "user_id", user.ID)
	...
	// Default: load last 50 replies (most recent)
	// If page > 1: load older replies (page 2 = oldest 50, page 3 = next 50, etc.)
	pgr := pager.NewPager(ctx, 50) // 50 replies per page

	// Count total replies
	totalReplies, err := h.orm.Message.
		Query().
		Where(message.ThreadIDEQ(id)).
		Count(ctx.Request().Context())

	if err != nil {
		logger.Error("Failed to count replies", "error", err, "message_id", id)
		return fail(err, "failed to count replies")
	}

	pgr.SetItems(totalReplies)
	logger.Info("Thread replies count", "message_id", id, "total_replies", totalReplies, "current_page", pgr.Page, "total_pages", pgr.Pages)

	// Load replies with pagination
	var replies []*ent.Message

	if pgr.Page == 1 {
		// Load most recent replies (last 50)
		// Calculate offset from the end
		offset := 0
		if totalReplies > pgr.ItemsPerPage {
			offset = totalReplies - pgr.ItemsPerPage
		}

		replies, err = h.orm.Message.
			Query().
			Where(message.ThreadIDEQ(id)).
			Order(ent.Asc(message.FieldCreatedAt)). // Always order by CreatedAt ASC
			Offset(offset).
			Limit(pgr.ItemsPerPage).
			WithUser().
			WithAttachments().
		...
		// Page 2 = oldest 50 replies (offset 0)
		// Page 3 = next 50 replies (offset 50), etc.
		offset := (pgr.Page - 2) * pgr.ItemsPerPage
	...
	logger.Info("Replies loaded", "message_id", id, "replies_count", len(replies), "page", pgr.Page)
	...
	r := ui.NewRequest(ctx)
	var buf bytes.Buffer
	// Pass pagination info to ThreadPanel component
	// hasMoreReplies: true if there are older replies to load
	// For page 1: hasMoreReplies = true if totalReplies > 50
	// For page > 1: hasMoreReplies = true if there are even older replies (page > 2)
	hasMoreReplies := false
	nextPage := 0

	if pgr.Page == 1 {
		// On first page, show "Load more" if there are more than 50 replies
		if totalReplies > pgr.ItemsPerPage {
			hasMoreReplies = true
			nextPage = 2 // Next page loads older replies
		}
	} else {
		// On subsequent pages, show "Load more" if there are even older replies
		if pgr.Page < pgr.Pages {
			hasMoreReplies = true
			nextPage = pgr.Page + 1 // Next page loads even older replies
		}
	}

	// Render thread panel with pagination info
	threadPanelNode := messengerComponents.ThreadPanel(r, parentData, replyData, hasMoreReplies, nextPage, int64(id))
	if err := threadPanelNode.Render(&buf); err != nil {
		logger.Error("Failed to render thread panel", "error", err)
		return fail(err, "failed to render thread panel")
	}

	logger.Info("Thread panel rendered", "message_id", id, "replies_count", len(replyData), "has_more", hasMoreReplies, "next_page", nextPage)
	return ctx.HTML(http.StatusOK, buf.String())
}
...
func (h *Messenger) MessageReply(ctx echo.Context) error {
	...
	if !isMember {
		logger.Warn("Message reply failed: user not a channel member", "parent_message_id", id, "channel_id", parentMsg.ChannelID, "user_id", user.ID)
		return echo.NewHTTPError(http.StatusForbidden, "you are not a member of this channel")
	}
	logger.Info("User verified as channel member", "channel_id", parentMsg.ChannelID)
	...
	if htmx.GetRequest(ctx).Enabled {
		...
		r := ui.NewRequest(ctx)
		// Check if reply is being added to thread panel (check hx-target header)
		hxTarget := ctx.Request().Header.Get("HX-Target")
		isThreadPanel := hxTarget == "thread-panel-replies" || hxTarget == "#thread-panel-replies"

		var replyNode Node
		if isThreadPanel {
			// Wrap reply in div for thread panel
			replyNode = Div(
				Class("bg-base-100 rounded-lg p-3 mb-2"),
				messengerComponents.MessageItem(r, messageData),
			)
		} else {
			// For backward compatibility (though form in main channel is removed)
			replyNode = messengerComponents.MessageItem(r, messageData)
		}

		var buf bytes.Buffer
		if err := replyNode.Render(&buf); err != nil {
